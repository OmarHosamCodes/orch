...
Constraints: one card; team-level quality (not per-project); top 10 projects; no donut; Team activity table uses one Activity column (description + meta row with tinted status chip · project); page filters untouched; dark Agency tokens; D01 instrument plates.
