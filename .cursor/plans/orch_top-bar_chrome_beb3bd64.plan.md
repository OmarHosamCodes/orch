---
name: Orch top-bar chrome
overview: "Replace the corner Eclipse dock and top-bar bell with a top-bar Eclipse companion: compact popover (composer, unread/running threads, Open Orch), then an expandable full thread with t3code-style Settle. Kill tools-in-+ and click-to-scope. Redesign tool activity. Inherit DESIGN.md; do not invent a new product identity."
todos:
  - id: od-comps
    content: "Open Design: two complete compact+expanded directions; user picks one"
    status: completed
  - id: hygiene-api
    content: lastReadAt + settle/unsettle/markRead/compact list APIs and tests
    status: completed
  - id: topbar-compact
    content: Replace bell with Eclipse trigger + compact popover; fix rail N more
    status: completed
  - id: expand-settle
    content: Expandable overlay + t3code Settle/Settled chrome
    status: completed
  - id: commands-kill-scope
    content: Ship / and @ commands; remove tools-in-+ and click-to-scope
    status: completed
  - id: thread-interiors
    content: CoT, activity, todo, citations, presence loading, expanded-only confetti
    status: completed
  - id: morph-kill-dock
    content: Retarget task-thread morph to top-bar Eclipse; delete corner dock
    status: completed
  - id: finish-checks
    content: Checks, detector, Agency+Canvas browser verification
    status: completed
isProject: false
---
